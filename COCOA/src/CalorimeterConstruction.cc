...
void CalorimeterConstruction::EndCap_Calorimeter()
{
	...
	long double r_inn                  = geometry.layer_inn_radius_ECAL[0][0];
        ...
        int         eta_segmentation_max   = GetMinOrMaxGeometry(
                                                geometry.eta_segmentation_ECAL,
                                                geometry.eta_segmentation_HCAL,
                                                false
                                             );

        int         phi_segmentation_max   = GetMinOrMaxGeometry(
                                                geometry.phi_segmentation_ECAL,
                                                geometry.phi_segmentation_HCAL,
                                                false
                                             );

        long double minDPhi                = GetMinOrMaxGeometry(
                                                geometry.layer_dphi_ECAL,
                                                geometry.layer_dphi_HCAL,
                                                true
                                             );
	...
	for (int ilow_layer = 0; ilow_layer < nLow_Layers; ilow_layer++)
	{
		...
		for (int ihigh_layer = 0; ihigh_layer < nHigh_Layers; ihigh_layer++)
		{
		    ...
		    Build_EndCap_CAL( eta_segmentation_max,
				      eta_segmentation_max / geometry.eta_segmentation_ECAL[ilow_layer][ihigh_layer],
                                      phi_segmentation_max,
                                      minDPhi,
	...
	for( auto dir_str : direction_name ) {
	    Build_EndCap_CAL( eta_segmentation_max,
			      eta_segmentation_max / 4, // Iron layer can be as coarse as we like in the eta direction.
                              phi_segmentation_max, 
			      minDPhi,
			      r_inn,
			      depth,
	...
	for (int ilow_layer = 0; ilow_layer < nLow_Layers; ilow_layer++)
	{
		...
		for (int ihigh_layer = 0; ihigh_layer < nHigh_Layers; ihigh_layer++)
		{
			...
			Build_EndCap_CAL( eta_segmentation_max,
					  eta_segmentation_max / geometry.eta_segmentation_HCAL[ilow_layer][ihigh_layer],
                                          phi_segmentation_max,
					  minDPhi,
...
void CalorimeterConstruction::Barrel_Calorimeter()
{
	...
	int         nLow_Layers             = geometry.number_of_pixels_ECAL.size();
        ...
        long double minDEta                 = 4.0 * config_json_var.max_eta_barrel / eta_segmentation_max;
	...
	for (int ilow_layer = 0; ilow_layer < nLow_Layers; ilow_layer++)
	{
		...
		for (int ihigh_layer = 0; ihigh_layer < nHigh_Layers; ihigh_layer++)
		{
			...
			Build_Barrel_CAL(eta_segmentation_max,
					 eta_segmentation_max / geometry.eta_segmentation_ECAL[ilow_layer][ihigh_layer],
                                         phi_segmentation_max,
					 minDEta,
					 minDPhi,
...
void CalorimeterConstruction::Build_Barrel_CAL(int EtaSegmentation, int cellMergeFactor, int PhiSegmentation, long double d_eta, long double d_phi, long double r_inn, long double r_out, long double previous_layers_delta_r, G4Material *Material_CAL, G4VisAttributes *VisAtt, const char *LV, const char *PL, int direction)
{
    ...
    //
        ...
        CheckMergeFactor( EtaSegmentation, cellMergeFactor );
	...
	const size_t nCellsPerDirection = 0.25 * EtaSegmentation;
	...
	for ( G4VSolid* cell : *ptr_cells_final ) {
		...
		CAL_LV               = new G4LogicalVolume( cell,
							    ...
							    LV );
		int iPhi = 0;
		for (auto angle = 0; angle < PhiSegmentation; angle++) //* loop that creates detector pixels in phi axis
		{
		        ++iPhi;
...
void CalorimeterConstruction::Build_EndCap_CAL(int EtaSegmentation, int cellMergeFactor, int PhiSegmentation, long double d_phi, long double r_inn_barrel, long double depth, long double previous_layers_depths, G4Material *Material_CAL, G4VisAttributes *VisAtt, const char *LV, const char *PL, int direction)
{
	...
	std::vector<G4VSolid*>  cells_maxNPixels;
	std::vector<G4VSolid*>* ptr_cells_final;
	const int nCells           = EtaSegmentation / 4;
	...
	for (size_t iEta = 0; iEta < ptr_cells_final->size(); ++iEta) {
	    G4LogicalVolume *endCap_LV_posdir = new G4LogicalVolume( ptr_cells_final->at(iEta),
								     Material_CAL,
								     LV );
	    for (auto iPhi = 0;  iPhi < PhiSegmentation; ++iPhi) {
		zRot = new G4RotationMatrix;
...
template <typename T>
T CalorimeterConstruction::GetMinOrMaxGeometry(
        const std::vector<std::vector<T>>& arr1,
        const std::vector<std::vector<T>>& arr2,
        bool chooseMin
) const {

    T arr1_extr = GetMinOrMax(arr1, chooseMin);
    T arr2_extr = GetMinOrMax(arr2, chooseMin);
    return (chooseMin) ? std::min(arr1_extr, arr2_extr) : std::max(arr1_extr, arr2_extr);
}
