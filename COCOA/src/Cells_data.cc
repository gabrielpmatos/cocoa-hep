...
Cells_data::Cells_data(bool is_high)
{
	...
	if (high)
        {
                Eta_Segmentation_Flatten = config_var.high_resolution.eta_segmentation_flatten;
                Phi_Segmentation_Flatten = config_var.high_resolution.phi_segmentation_flatten;
        }
        else
        {
	...
	ilay_num = Eta_Segmentation_Flatten.size();
